...
*/
        ...
        */
        ...
        XposedHelpers.findAndHookMethod(clazzPanelBar != null ? clazzPanelBar : clazz2, "onAttachedToWindow", new XC_MethodHook() {
            ...
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                ...
                ensureInitialized((View) param.thisObject);
            }
        });

        // --- Zygisk Next / KSU Next compatibility fix - start --- //
        // On Magisk's native Zygisk, module injection happens early enough that this module
        // never misses the status bar's *first* onAttachedToWindow() call. Zygisk Next injects
        // later and with more variable timing (it ptrace-attaches via a userspace daemon rather
        // than patching zygote itself), so on some boots the very first onAttachedToWindow()
        // call already happened before our hook was registered. Since that callback only fires
        // once per view lifetime, we'd otherwise miss it entirely and the status bar would keep
        // rendering unhooked (default/OEM sizing) until some unrelated config-change broadcast
        // happened to fire onConfigurationChanged() later - which is exactly the "bigger for a
        // few seconds, then reverts" symptom users see on KSU Next + Zygisk Next.
        //
        // dispatchDraw() runs every single frame regardless of when we attached, so hooking it
        // gives us a reliable, near-immediate fallback point to self-heal from a missed attach
        // without waiting on an unrelated system event. ensureInitialized() is idempotent, so
        // this is effectively free once the normal path has already run.
        Method dispatchDrawMethod = XposedHelpers.findMethodExactIfExists(clazz2, "dispatchDraw", Canvas.class);
        if (dispatchDrawMethod != null) {
            XposedBridge.hookMethod(dispatchDrawMethod, new XC_MethodHook() {
                @Override
                protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
                    if (mStatusBarView == null) {
                        log("[SpSd - SelfHeal] Missed initial onAttachedToWindow, recovering on first draw");
                        ensureInitialized((View) param.thisObject);
                    }
                }
            });
        }
        // --- Zygisk Next / KSU Next compatibility fix - end --- //
    ...
    /**
     * Idempotent init/sync routine. Safe to call from any hook that hands us a live status bar
     * View, however early or late that hook actually fires. Creating the overlay service and
     * listeners only happens once; everything else (bar shifting/alpha "OEM look" fixes) is
     * cheap to reapply every time this is called.
     */
    private void ensureInitialized(View statusBarView) {
